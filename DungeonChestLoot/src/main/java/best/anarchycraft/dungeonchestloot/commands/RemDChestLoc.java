...
public class RemDChestLoc implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (player.hasPermission("best.anarchycraft.dungeonchestloot.chests")){
            if (checkArgsSyntax(args, player)) {
                player.sendMessage("" + ChatColor.AQUA + ChatColor.ITALIC + "Performing this will" + ChatColor.RED + ChatColor.ITALIC + " remove " + ChatColor.AQUA + ChatColor.ITALIC + "the clicked block from the config.");
                player.sendMessage("" + ChatColor.BLUE + ChatColor.ITALIC + "Click on the Block under the chest position.");
                Dungeonchestloot.INSTANCE.enableListener(player, 1, args[0], args[1]);
                return true;
            }
        }
        else{
